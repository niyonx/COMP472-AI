...
def find_possible_paths(puzzle, opened= [], closed=[], columns= 4, rows= 2):
    ...
    if(temp_moving_token >= 0):
        paths.append((move(puzzle, zero_idx, temp_moving_token), COST.REGULAR, puzzle, puzzle[temp_moving_token]))
    ...
    if(temp_moving_token <= last_idx):
        ...
        if((zero_idx + 1) % columns != 0): 
            paths.append((move(puzzle, zero_idx, temp_moving_token), COST.DIAGONAL, puzzle, puzzle[temp_moving_token]))
    ...
    if(zero_idx == 0):
        ...
        paths.append((move(puzzle, zero_idx, temp_moving_token), COST.WRAPPING, puzzle, puzzle[temp_moving_token]))
        # WRAP DOWN
        if(rows > 2):
    ...
    for path in paths:
        open_idx = get_tuple_index(opened, 0, path[0])
